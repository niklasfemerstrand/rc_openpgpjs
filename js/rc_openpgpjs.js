...
if (window.rcmail) {
  rcmail.addEventListener('init', function (evt) {
    ...
    if (rcmail.env.action === "compose" || rcmail.env.action === "preview") {
      ...
      $("#mailtoolbar").append("<a href='#' class='button' id='openpgp_js' onclick=\"return rcmail.command('open-key-manager','',this,event)\">Key Manager</a>");
...
function update_tables() {
  ...
  for (var i = 0; i < openpgp.keyring.publicKeys.length; i++) {
    ...
    var result = "<tr class='clickme' onclick='displayPub(" + i + ");'><td>0x" +
      ...
      "</td><td>" +
      "<a href='#' onclick='if (confirm(\"Delete this public key?\")) { openpgp.keyring.removePublicKey(" + i + "); update_tables(); }'>"+ 
      rcmail.gettext('delete','rc_openpgpjs') +"</a>" +
      "</td></tr>";
  ...
  for (var i = 0; i < openpgp.keyring.privateKeys.length; i++) {
    for (var j = 0; j < openpgp.keyring.privateKeys[i].obj.userIds.length; j++) {
      ...
      "</td><td>" +
      "<a href='#' onclick='if (confirm(\"Delete this private key?\")) { openpgp.keyring.removePrivateKey(" + i + "); update_tables(); }'>"+ 
      rcmail.gettext('delete', 'rc_openpgpjs') +"</a>" +
      "</td></tr>");
    }
  }
